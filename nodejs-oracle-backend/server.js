...
VALUES (:1, :2, :3, :4, :5, :6, :7, :8)`,
      ...
      [SSN, FNAME, LNAME, EMAIL, DNO, PNO, STUEMP, PW],
...
app.post("/book-room", async (req, res) => {
  ...
  try {
    ...
    console.log(result); // ตรวจสอบค่าที่ได้รับ

    const reserverId = result.outBinds[0][0]; // ปรับการเข้าถึงค่า
  ...
  try {
    const result = await executeQuery(
      `SELECT e.SSN, e.FNAME, e.LNAME, e.EMAIL, d.DNAME, p.PNAME, s.STATUSEMPNAME, e.DNO, e.PNO, e.STUEMP
       ...
       JOIN POSITION p ON e.PNO = p.PNUMBER
       JOIN STATUSEMP s ON e.STUEMP = s.STATUSEMPID
       WHERE e.SSN = :ssn AND e.PW = :password`,
      [ssn, password],
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
    ...
    // ตรวจสอบสถานะพนักงาน
    if (user.STATUSEMPNAME !== "Active") {
      return res.status(403).json({ error: "Account is not active" });
    }

    // ตรวจสอบการล็อกบัญชี
    const lockCheck = await executeQuery(
      `SELECT * FROM LOCKEMP WHERE ESSN = :ssn AND LOCKDATE > SYSDATE - 1`,
      [ssn],
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
    ...
    res.json({
      user: {
        ...
        position: user.PNAME,
        status: user.STATUSEMPNAME,
      },
      menuAccess: menuAccess.rows,
    });
  } catch (err) {
